  ...
  return (
    <tr
      className={`hover:bg-[var(--ds-bg-hover)] transition-colors group cursor-pointer ${
        isRowSelected ? (isManualDraft ? 'bg-amber-500/5' : 'bg-teal-500/5') : ''
      }`}
    ...
    >
      ...
      <td className="px-4 py-4" onClick={handleCellClick}>
        {isManualDraft ? (
          ...
          >
            <div className="p-2 bg-[var(--ds-bg-elevated)] rounded-lg text-[var(--ds-text-secondary)] group-hover:text-teal-200 transition-colors">
              <FileText size={16} />
            </div>
            <span
              className="font-medium text-[var(--ds-text-primary)] group-hover:text-teal-200 transition-colors truncate max-w-50"
              title={template.name}
            >
              {template.name}
            </span>
        ...
        <span
          className={`inline-flex items-center rounded-md border px-2 py-1 text-xs font-medium ${
            template.category === 'UTILIDADE'
              ? 'bg-teal-500/10 text-[var(--ds-status-success-text)] border-teal-500/20'
