/**
 * Bootstrap da instância BlueTick.
 * Configura settings iniciais após migrations.
 *
 * BlueTick usa MASTER_PASSWORD para auth, não Supabase Auth.
 ...
 * BlueTick usa MASTER_PASSWORD via env var, não Supabase Auth.
...
export async function bootstrapInstance({
  ...
  const companyName = adminName?.trim() || emailNorm.split('@')[0] || 'BlueTick';
