...
const CACHE_VERSION = 'bluetick-v1'
  ...
  event.waitUntil(
    caches.keys()
      .then((cacheNames) => {
        return Promise.all(
          cacheNames
            .filter((name) => name.startsWith('bluetick-') && name !== STATIC_CACHE && name !== DYNAMIC_CACHE)
            .map((name) => {
