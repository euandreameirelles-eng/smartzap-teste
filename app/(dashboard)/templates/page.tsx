...
const StatusBadge = ({ status, approvedCount, totalCount }: { status: string; approvedCount?: number; totalCount?: number }) => {
  ...
  if (isComplete) {
    return (
      <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium border bg-teal-500/10 text-teal-300 border-teal-500/20">
        Concluído
      </span>
    );
  }
    ...
    <Page>
      <PageHeader>
        <div>
          ...
          <PageDescription>
            {activeTab === 'flows'
              ? 'Crie e monitore MiniApps do WhatsApp, e mapeie respostas para campos do BlueTick.'
              : activeTab === 'forms'
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`rounded-full border px-4 py-2 text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'meta'
            ? 'border-teal-400/40 bg-teal-500/10 text-[var(--ds-status-success-text)]'
        ...
        >
          ...
          MiniApps
          <span className="rounded-full bg-teal-500/20 px-1 py-px text-[8px] font-semibold uppercase tracking-wider text-[var(--ds-status-success-text)] border border-teal-500/30">
            beta
          </span>
        </button>
                              ...
                              {editingProjectId === project.id ? (
                                <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                  <input
                                    ...
                                    autoFocus
                                    className="px-2 py-1 rounded-lg border border-teal-500 bg-[var(--ds-bg-elevated)] text-[var(--ds-text-primary)] text-sm font-medium focus:outline-none focus:ring-2 focus:ring-teal-500/30"
                                  />
                                  <button
                                    onClick={handleSaveEdit}
                                    disabled={isUpdating}
                                    className="p-1.5 rounded-lg text-teal-400 hover:bg-teal-500/10 disabled:opacity-50"
                                    title="Salvar"
                                  >
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <div className="flex-1 w-24 bg-[var(--ds-bg-surface)] rounded-full h-1">
                                <div
                                  className="bg-teal-500 h-1 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.5)]"
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                title="Renomear"
                                className="p-2 rounded-lg text-[var(--ds-text-secondary)] hover:text-teal-400 hover:bg-teal-500/10"
                              >
                                <Pencil size={16} />
                              </button>
                              <button
