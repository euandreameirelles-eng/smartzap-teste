...
function buildSupportPacketText(params: {
	...
	const lines: string[] = []
	lines.push(`BlueTick · Support Packet · ${new Date().toLocaleString('pt-BR')}`)
...
function buildMissingPermissionsSteps(params: { objectLabel: string; objectId: string }) {
	return [
		...
		'No app BlueTick: cole esse token e os IDs corretos em Ajustes → Credenciais WhatsApp.',
...
function buildMissingScopesSteps(missing: string[]) {
	...
	return [
		...
		'No BlueTick: atualize o token em Ajustes → Credenciais WhatsApp e rode o diagnóstico novamente.',
	]
}
	...
	const nextSteps = isMissingPermissionsOrWrongId
		...
		: isTokenInvalid
			? [
				...
				'Atualize as credenciais no BlueTick e tente novamente.',
			]
			: [
...
function buildReportText(
	...
	lines.push(`BlueTick · Diagnóstico Meta/WhatsApp · ${new Date().toLocaleString('pt-BR')}`)
...
export async function GET() {
	...
	if (sub.ok) {
		// override_callback_uri apontando para a URL do BlueTick também é configuração válida —
		// eventos são roteados via override mesmo sem subscribed_fields explícito no subscribed_apps.
		const overrideIsBlueTick =
			sub.overrideCallbackUri != null && sub.overrideCallbackUri === webhookUrl
		const webhookActive = sub.messagesSubscribed || overrideIsBlueTick
		...
		} else if (overrideIsBlueTick) {
			statusLabel = 'pass'
			messageLabel = `Ativo via override_callback_uri → ${sub.overrideCallbackUri}`
		} else {
			statusLabel = 'fail'
			messageLabel = 'Inativo — override_callback_uri não aponta para o BlueTick e messages não inscrito'
		}
